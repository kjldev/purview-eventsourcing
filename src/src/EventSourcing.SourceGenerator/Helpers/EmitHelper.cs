...
static class EmitHelper
{
    ...
    static void GenerateEventClass(StringBuilder sb, AggregateEventMethodInfo method)
    {
        ...
        {
            sb.AppendLine(
                $"\t\tpublic {prop.TypeName} {prop.PropertyName} {{ get; set; }} = default!;"
            );
        }
    ...
    static void GenerateRegisterEvents(StringBuilder sb, AggregateInfo info, string indent)
    {
        ...
        {
            sb.AppendLine(
                $"{indent}\t\tRegister<global::{eventsNamespace}.{method.EventName}>(Apply);"
            );
        }
    ...
    )
    {
        ...
        sb.AppendLine(
            $"{indent}\tvoid Apply(global::{eventsNamespace}.{method.EventName} {eventParameterName})"
        );
        sb.AppendLine($"{indent}\t{{");
        ...
        {
            sb.AppendLine(
                $"{indent}\t\t{prop.PropertyName} = {eventParameterName}.{prop.PropertyName};"
            );
        }
    ...
    )
    {
        ...
        for (var i = 0; i < method.Parameters.Count; i++)
        {
            if (i > 0)
                paramList.Append(", ");
            paramList.Append(
                $"{method.Parameters[i].TypeName} {method.Parameters[i].ParameterName}"
            );
        }
