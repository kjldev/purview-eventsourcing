...
{
    const string GenerateAggregateAttributeName =
        "Purview.EventSourcing.Aggregates.GenerateAggregateAttribute";
        ...
        var isDisabled = context.AnalyzerConfigOptionsProvider.Select(
            static (opts, _) =>
            {
                opts.GlobalOptions.TryGetValue(
                    "build_property.DisableEventSourcingSourceGenerator",
                    out var val
                );
    ...
    )
    {
        ...
        var eventAttributeSymbol = compilation.GetTypeByMetadataName(
            GenerateAggregateEventAttributeName
        );
        ...
        if (
            aggregateBaseSymbol is null
            || !InheritsFromAggregateBase(classSymbol, aggregateBaseSymbol)
        )
        {
            diagnostics.Add(
                Diagnostic.Create(
        ...
        {
            foreach (
                var methodSymbol in attributedMethods.Where(method =>
                    method.Name == duplicatedMethodName
                )
            )
            {
                diagnostics.Add(
    ...
    {
        return ctx.TargetSymbol is not IMethodSymbol methodSymbol
    ...
    )
    {
        ...
        foreach (var attribute in methodSymbol.GetAttributes())
        {
            ...
            if (
                ...
                || attributeClass.ToDisplayString() != GenerateAggregateEventAttributeName
            )
                continue;
    ...
    static bool InheritsFromAggregateBase(
        INamedTypeSymbol classSymbol,
        INamedTypeSymbol aggregateBaseSymbol
    )
    {
