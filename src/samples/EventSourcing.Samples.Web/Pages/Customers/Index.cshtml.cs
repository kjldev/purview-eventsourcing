...
public sealed class IndexModel(IQueryableEventStore<CustomerAggregate> store) : PageModel
{
	...
	[BindProperty(SupportsGet = true)] public bool? ActiveFilter { get; set; }
	...
	public async Task OnGetAsync()
	{
		...
		Func<IQueryable<CustomerAggregate>, IQueryable<CustomerAggregate>> orderBy = (SortBy, SortDir) switch
		{
			...
			("version", "desc") => q => q.OrderByDescending(c => c.Details.SavedVersion),
			("version", _) => q => q.OrderBy(c => c.Details.SavedVersion),
