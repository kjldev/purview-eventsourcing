...
public sealed class IndexModel(IQueryableEventStore<InventoryAggregate> store) : PageModel
{
	...
	public async Task OnGetAsync()
	{
		...
		Func<IQueryable<InventoryAggregate>, IQueryable<InventoryAggregate>> orderBy = (SortBy, SortDir) switch
		{
			...
			("onhand", "desc") => q => q.OrderByDescending(i => i.QuantityOnHand),
			("onhand", _) => q => q.OrderBy(i => i.QuantityOnHand),
			("available", "desc") => q => q.OrderByDescending(i => i.AvailableQuantity),
			("available", _) => q => q.OrderBy(i => i.AvailableQuantity),
