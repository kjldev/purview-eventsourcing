...
public sealed class StockTransferServiceTests
{
    ...
    public async Task TransferAsync_GivenNullSource_ReturnsFail(CancellationToken cancellationToken)
    {
        var store = Substitute.For<IQueryableEventStore>();
        store
            .GetAsync<InventoryAggregate>("missing", null, cancellationToken)
    ...
    )
    {
        ...
        store.GetAsync<InventoryAggregate>("inv-1", null, cancellationToken).Returns(source);
        store
            .GetAsync<LocationAggregate>("LOC-001", null, cancellationToken)
            .Returns(sourceLocation);
        store
            .GetAsync<LocationAggregate>("missing", null, cancellationToken)
    ...
    )
    {
        ...
        store
            ...
            .Returns((LocationAggregate?)null);
        store
            .GetAsync<LocationAggregate>("LOC-002", null, cancellationToken)
