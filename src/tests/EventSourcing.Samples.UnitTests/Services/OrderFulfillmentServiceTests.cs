...
public sealed class OrderFulfillmentServiceTests
{
    ...
    )
    {
        ...
        store
            .GetAsync<CustomerAggregate>("missing", null, cancellationToken)
    ...
    )
    {
        ...
        store.GetAsync<CustomerAggregate>(customer.Id(), null, cancellationToken).Returns(customer);
        store
            .GetAsync<InventoryAggregate>("missing-inv", null, cancellationToken)
    ...
    )
    {
        ...
        store.GetAsync<CustomerAggregate>(customer.Id(), null, cancellationToken).Returns(customer);
        store
            .GetAsync<InventoryAggregate>(inventory.Id(), null, cancellationToken)
            .Returns(inventory);
