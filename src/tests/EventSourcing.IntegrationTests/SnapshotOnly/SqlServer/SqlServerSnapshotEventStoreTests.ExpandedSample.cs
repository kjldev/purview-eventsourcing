...
partial class SqlServerSnapshotEventStoreTests
{
    ...
    )
    {
        ...
        List<PersistenceAggregate> aggregates = [];
        await foreach (
            var aggregate in eventStore.GetListEnumerableAsync(cancellationToken: cancellationToken)
        )
            aggregates.Add(aggregate);
    ...
    )
    {
        ...
        bool firstSave = await context.EventStore.SaveAsync(
            aggregate,
            cancellationToken: cancellationToken
        );
        ...
        bool secondSave = await context.EventStore.SaveAsync(
            aggregate,
            cancellationToken: cancellationToken
        );
